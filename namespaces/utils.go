...
func checkNamespaceFlags(container *libcontainer.Config) error {
	...
	if ((cloneFlags & syscall.CLONE_NEWNET) == 0) &&
		...
		return errors.New("unable to apply network parameters without network namespace")
	}
	if (cloneFlags & syscall.CLONE_NEWNS) == 0 {
		if container.MountConfig != nil {
			return errors.New("mount_config is set without mount namespace")
		}
		if container.RestrictSys {
			return errors.New("unable to restrict access to sysctl without mount namespace")
		}
	}
	if (cloneFlags & syscall.CLONE_NEWUTS) == 0 {
		return errors.New("unable to set the hostname without UTS namespace")
	}
